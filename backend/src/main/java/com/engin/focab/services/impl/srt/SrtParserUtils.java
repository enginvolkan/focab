...
//		return (hours > 9 ? hours + ":" : "0" + hours + ":") + (minutes > 9 ?  minutes + ":" : "0" + minutes + ":") + (seconds > 9 ?  seconds : "0" + seconds) + "," + (millis > 99 ? millis : millis > 9 ? "0" + millis : millis >= 0 ? "00" + millis : "000");
	 ...
	 * Obs. Deve ser configurado no load do arquivo para utilizar Node #{@link DefaultSrtParserService#getSubtitlesFromFile(String, boolean, boolean)}
	 * @param subtitle
	 * @param timeMillis
	 * @return
	 */
...
//		
	 ...
	 * @return
	 */
	public static boolean speedSynchronization (final ArrayList<SubtitleModel> listSubtitles, long timeInMillis, File fileOut) {
...
//			fileOut.mkdirs();		
		...
		try (	
			...
			for (int i= 0; i < length; i ++) {
				...
				bos.newLine();
				bos.write(SrtParserUtils.millisToText(subtitle.timeIn + timeInMillis, subtitle.timeOut + timeInMillis));
				bos.newLine();
