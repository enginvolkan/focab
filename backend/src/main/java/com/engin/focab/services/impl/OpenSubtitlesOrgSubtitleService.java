...
public class OpenSubtitlesOrgSubtitleService implements SubtitleService {
	...
	private OpenSubtitlesClient getClient() throws MalformedURLException, XmlRpcException {
		if (this.client == null) {
...
//			this.client = new OpenSubtitlesClientImpl(config);
	...
	/* (non-Javadoc)
	 * @see com.engin.focab.services.impl.SubtitleService#getASubtitleByImdbId(java.lang.String)
	 */
	@Override
	public SubtitleFile getASubtitleByImdbId(String id) throws XmlRpcException, MalformedURLException {
		SubtitleFile subTitle = null;
		...
		if (subtitleInfo.isPresent()) {
			List<SubtitleFile> subtitleFiles = getClient().downloadSubtitles(subtitleInfo.get().getSubtitleFileId());
